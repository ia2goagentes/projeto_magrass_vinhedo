...
import { DailyEntry, WeeklyAdMetric } from "@/lib/types";
...
export default function HistoricoPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mt-6 flex w-fit gap-1 rounded-2xl border border-border-hairline bg-surface-card p-1.5">
        <button
          onClick={() => setTab("diario")}
          className={`rounded-xl px-4 py-2 text-sm font-medium transition-colors ${
            tab === "diario" ? "bg-accent text-white" : "text-ink-secondary hover:bg-ink-primary/5"
          }`}
        >
          Lançamentos diários
        </button>
        <button
          ...
          className={`rounded-xl px-4 py-2 text-sm font-medium transition-colors ${
            tab === "semanal" ? "bg-accent text-white" : "text-ink-secondary hover:bg-ink-primary/5"
          }`}
        >
      ...
      <div className="mt-4">{tab === "diario" ? <DailyHistory /> : <WeeklyHistory />}</div>
    </div>
  );
}
